...
import { connCafeMeta } from '../../../../utils/connection';
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const handleCase: ResponseFuncs = {
    ...
    GET: async (req2: NextApiRequest, res2: NextApiResponse) => {
      ...
      const { CafeMeta } = await connCafeMeta(); // connect to database
      if (id) {
        // @ts-ignore
        const response = await CafeMeta.find({ tokenId: id }).catch(catcher);
        // res2.statusCode= 200;
    ...
    POST: async (req3: NextApiRequest, res3: NextApiResponse) => {
      ...
      // @ts-ignore
      res3.json(await CafeMeta.create(req3.body).catch(catcher));
      res3.end();
      // console.log(res3);
      return res3;
    },
